...
WHERE t.transfer_date >= CURRENT_DATE - INTERVAL '1000 days'
...
HAVING SUM(s.sale_price * s.quantity) > 100000
ORDER BY total_spent DESC;
...
SELECT pr.name,
       SUM(s.sale_price * s.quantity) - SUM(p.purchase_price * s.quantity) AS profit
FROM sales s
...
GROUP BY
    c.client_id
HAVING
    SUM(s.quantity * s.sale_price) > 100000
ORDER BY
    total_spent DESC;
...
SELECT
    EXTRACT(YEAR FROM s.sale_date) || '-Q' || EXTRACT(QUARTER FROM s.sale_date) AS quarter,
